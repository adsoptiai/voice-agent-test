...
## 問題描述
Connection failed: Failed to execute 'setRemoteDescription' on 'RTCPeerConnection': Failed to parse SessionDescription. { Expect line: v=
...
export async function getSessionToken() {
  ...
  try {
    // 使用 fetch 直接呼叫 API (避免 SDK 版本問題)
    const response = await fetch("https://api.openai.com/v1/realtime/sessions", {
      method: "POST",
      headers: {
        "Authorization": `Bearer ${apiKey}`,
        "Content-Type": "application/json",
        "OpenAI-Beta": "realtime=v1" // 加入 beta header
      },
      body: JSON.stringify({
...
### 3. 更新 page.tsx
使用 WebSocket 直接連接，避免 WebRTC 問題。
...
**問題 2: WebSocket connection failed**  
- 解決：檢查 API key 權限和網路連接

**問題 3: Failed to access microphone**
- 解決：確保瀏覽器有麥克風權限，使用 HTTPS 或 localhost
